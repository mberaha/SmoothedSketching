...
MAXMEM = 10000
NDATA = [25000, 100000, 250000, 500000]
MODELS = ["DP", "NGG"]
...
function frequency_simulation(data, test_data)
    ...
    for (i, J) in enumerate(Js)
        ...
        for (k, test_cnt) in uniq2cnt_test
            ...
            maes_debiased[i, mae_bucket] += test_cnt * abs(cnt - debiased_cms_est) / ndata_by_bucket[mae_bucket]
    ...
    CSV.write("results/bigram_maes.csv", maes_df)
end
...
function cardinality_simulation(true_dataset)
    ...
    df[!, "ndata"] = NDATA
    df[!, "true_k"] = k_true
    CSV.write("results/bigram_cardinality.csv", df)
end
